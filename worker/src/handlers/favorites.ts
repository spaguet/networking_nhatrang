...
/** GET /api?action=getFavoriteCounts */
export async function handleGetFavoriteCounts(
  ...
  try {
    ...
    const favorites = favoritesArr.map((item) => ({
      ...
      total: item.total,
      favoritedByMe: item.userIds.indexOf(validation.userId) !== -1,
    }));
...
export async function handleToggleFavorite(
  ...
  const tgId = Number(validation.userId);
